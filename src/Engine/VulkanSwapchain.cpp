...
VulkanSwapchain VulkanSwapchain::createSwapchain(const VulkanDevice &dev, const VkSwapchainCreateInfoKHR &createInfo)
{
	...
	auto result = dev.getFunctionSet().vkCreateSwapchainKHR(dev.getHandle(), &createInfo, nullptr, &swapchain);
...
VulkanSwapchain::VulkanSwapchain(const VulkanDevice &dev, const VkSwapchainKHR &swapchain, const VkSwapchainCreateInfoKHR &createInfo)
{
	...
	vk.vkGetSwapchainImagesKHR(dev.getHandle(), this->handle, &imageCount, nullptr);
	this->images.resize(imageCount);
	vk.vkGetSwapchainImagesKHR(dev.getHandle(), this->handle, &imageCount, this->images.data());
	...
	for (int i = 0; i < this->images.size(); i++)
	{
		...
		if (vk.vkCreateImageView(dev.getHandle(), &cinfo, nullptr, &this->imageViews[i]) != VK_SUCCESS) {
